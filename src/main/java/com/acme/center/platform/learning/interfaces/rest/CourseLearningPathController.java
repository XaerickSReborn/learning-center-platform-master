...
public class CourseLearningPathController {
    ...
    public ResponseEntity<LearningPathItemResource> addTutorialToCourseLearningPath(@PathVariable Long courseId, @PathVariable Long tutorialId) {
        var tutorialIdValueObject = new TutorialId(tutorialId);
        courseCommandService.handle(new AddTutorialToCourseLearningPathCommand(tutorialIdValueObject, courseId));
        var getLearningPathItemByCourseIdAndTutorialIdQuery = new GetLearningPathItemByCourseIdAndTutorialIdQuery(courseId, tutorialIdValueObject);
